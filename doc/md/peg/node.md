...
Since the request is for a substring, this is inherently expensive, we let the
Palimpsest handle any internal changes which result\.
...
##### predicator\(node, pred\)

We allow predicates to be a string, which we compare with `.tag`\.

Otherwise we call it \(if we can\)\.  Invalid predicates are always `false`\.
