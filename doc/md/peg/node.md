...
### \[\#Todo\] common table: \.g, :G\(\)

It would be straightforward to smuggle in global state with a constant capture
of a table in `match`\.

Pros

  -  The big one is getting to check for `.v` locally, with a logic\-free
      operation, in two lookups\.

  -  There is a pattern I get into where the top Node starts wearing all the
      clothes, so to speak, just because I, and the children, can both get at
      it\.  But really, these are global state\.

Cons:

  - It's another thing to have to provide?  That part seems easy though,
      there's only one, so as long as we remove them all when rebasing, we know
      the one we can find with a parent walk is the one\.

I'm writing the Clade without 'magic': the magic way would be an `__index`
which finds this table for you if the slot is nil\.

I'd rather call it `.g` and offer `node:G()` to retrieve it\.

Yeah\.  I like this\.  Because we don't even have to pass `.g` in as a capture,
in fact, we *don't need to have one* if the Node isn't using it, and that
seems key: it's a basic enough pattern to provide with the base genre, but
I imagine many occasions when we just don't need one\.

One extra key/value pair per node isn't *that* much bulk, but I'm taking pains
to only get what I want to pay for here\.

This is easy, I'm going to do it right now\.
...
### Node:G\(\)
  ...
  This finds `.g`, a field defined as an optional table shared by the entire
AST\.

The Clade basis doesn't use `.g` for anything, and only creates it if it can't
find it\.  It will `nil` out the field during rebasing, that's it\.
...
### Node:adjust\(v: i\): skew: i, v\_new: i
...
Finding matching Nodes within, after, and \(eventually\) before the given Node\.

All of these take a predicate, in the following flavors\.
...
PEG `cat` rule\) and sometimes have just one element\.
